...
const features = [
    ...
    },
    {
        ...
        description: 'Magic links setup, login with Google walkthrough, save user in MongoDB/Supabase, private/protected pages & API calls',
        svg: (
            <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={1.5} stroke='currentColor' className='w-6 h-6'>
                <path
                    strokeLinecap='round'
    ...
    {
        title: 'Style',
        description: 'Components, animations & sections (like this features section), 20+ themes with daisyUI, automatic dark mode',
        svg: (
            <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={1.5} stroke='currentColor' className='w-6 h-6'>
                <path
                    strokeLinecap='round'
...
const Item = ({ feature, isOpen, setFeatureSelected }: { index: number; feature: Feature; isOpen: boolean; setFeatureSelected: () => void }) => {
...
const Media = ({ feature }: { feature: Feature }) => {
    ...
    if (type === 'video') {
        ...
        return <Image src={path} alt={alt} className={`${style} object-cover object-center`} width={size.width} height={size.height} />;
    } else {
